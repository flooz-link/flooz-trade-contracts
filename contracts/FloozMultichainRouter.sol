...
contract FloozMultichainRouter is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IFloozMultiChainRouter {
    ...
    function executeOneInchSwap(ExternalSwapData calldata swapData)
        ...
        isValidReferee(swapData.referee)
    {
        require(swapData.fromToken != swapData.toToken, "FloozRouter: INVALID_TRADE");
        address referee = _getReferee(swapData.referee);
    ...
    function _withdrawFeesAndRewards(
        ...
        if (tokenReward == address(0)) {
            ...
            if (referralReward > 0) {
                bool success = payable(referee).send(referralReward);
                if (!success) {
                    IWETH(WETH).deposit{value: referralReward}();
                    IWETH(WETH).transfer(referee, referralReward);
                }
